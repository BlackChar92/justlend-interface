...
export function toBigIntLE(buf: Buffer): bigint {
  if (buf.length > 1024) {
    throw new Error(`toBigIntLE: Buffer too large (${buf.length} bytes) — possible overflow attack`);
  }
...
export function toBigIntBE(buf: Buffer): bigint {
  if (buf.length > 1024) {
    throw new Error(`toBigIntBE: Buffer too large (${buf.length} bytes) — possible overflow attack`);
  }
  if (process.browser || converter === undefined) {
