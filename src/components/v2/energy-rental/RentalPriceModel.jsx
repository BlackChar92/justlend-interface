...
@inject('lend')
@observer
class RentalPriceModel extends Component {
  constructor(props) {
    super(props);
    this.state = {
      mobile: isMobile().any,
  ...
  getEchartsOption() {
    ...
    const options = getChartOptions({ theme, dataList, isMobile: mobile, labelStatus });
    return options;
  }
...
function getChartOptions({ theme, dataList, isMobile, labelStatus }) {
  ...
  const serialData = xData.map((item, index) => {
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        return `<div class="chart-tooltip">
            <div class="stat">
              <span class="stat-title">${Intl.get('energy_rental.market_data.chart_tooltip_price')}</span>
              <span class="stat-value green-color">${data.trx1wEnergyShow} sun/${Intl.get('strx.energy_day2')}</span>
            </div>
            <div class="stat">
              <span class="stat-title">${Intl.get('energy_rental.market_data.chart_tooltip_utilization')}</span>
              <span class="stat-value">${data.occupancyRateShow}</span>
            </div>
          </div>`;
      }
    },
    xAxis: {
      ...
      nameTextStyle: {
        ...
        verticalAlign: 'top',
        fontWeight: 400,
        padding: isMobile ? [13, 12, 0, 0] : [12, 16, 0, 0]
      }
    },
    yAxis: {
      type: 'value',
