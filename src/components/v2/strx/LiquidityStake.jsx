...
class LiquidityStake extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      sTRX2TRX: '--',
      inputsTRX: '',
      showStrxBalance: window.sessionStorage.getItem('show_strx_balance') ? true : false, // true strx, false trx,
      ...
      claimSubmit: false,
      fromFinance: false,
      apyRange: window.sessionStorage.getItem('apy_range') || '7D' // 6H, 7D
    };
  }
  ...
  claimAll = async () => {
    ...
    if (txID) {
      // Optimistic update: tx success means on-chain claimable is now 0.
      // Zero it immediately so the button becomes disabled without waiting
      // for the backend indexer to catch up.
      this.props.strx.setData({ accountCanClaimAmount: 0 }, 'userData');
      setTimeout(async () => {
        await this.props.strx.getMarketData();
        await this.props.strx.getUserData();
        await this.props.strx.getUserTrxBalance();
  ...
  exchangeShowStrxBalance = () => {
    ...
    !showStrxBalance
      ? window.sessionStorage.setItem('show_strx_balance', true)
      : window.sessionStorage.removeItem('show_strx_balance');
  ...
  getAnnouncementUrl = () => {
    const announcementUrl = Config.announceForUSDDSTRXNew;
  ...
  onApyRangeClick = () => {
    const { apyRange } = this.state;
    const newApyRange = apyRange === '7D' ? '6H' : '7D';
    this.setState({
      apyRange: newApyRange
    });
    window.sessionStorage.setItem('apy_range', newApyRange);
  };
  ...
  render() {
    ...
    const currentApy = apyRange === '7D' ? avgApy : avgApy6h;
            ...
            <div className="top-header">
              ...
              <div className="info">
                ...
                <div className="j-announce">
                  ...
                  >
                    {intl.get('strx.usdd_annoucement_new')}
                  </a>
                ...
                BigNumber(accountCanClaimAmount).eq(0) ? (
                  <div className="apy-box-nostake">
                    <div className="apy-title tooltip-text-wrap">
                      <TooltipText
                        ...
                        title={
                          <>
                            {intl.get(
                              apyRange === '6H' ? 'strx.stake_staking_apy_tip1' : 'strx.stake_staking_apy_tip2'
                            )}{' '}
                            <LinkButton href={learnUrl} target="_blank" rel="noopener noreferrer">
                    ...
                    <div className="apy-value">
                      <>{formatNumber(currentApy * 100, 2, { miniText: 0.01 })}%</>
                      <div className="apy-range" onClick={this.onApyRangeClick}>
                        {intl.get(apyRange === '6H' ? 'strx.stake_staking_6hours_apy' : 'strx.stake_staking_7days_apy')}
                      </div>
                    </div>
                    ...
                    {BigNumber(accountWithDrawAmount).eq(0) && BigNumber(accountCanClaimAmount).eq(0) ? (
                      <div className="apy-only-stake">
                        <div className="column">
                          ...
                          <div className="apy-value">
                            <strong className="value">{formatNumber(currentApy * 100, 2, { miniText: 0.01 })}%</strong>
                            <div className="apy-range" onClick={this.onApyRangeClick}>
                              {intl.get(
                                apyRange === '6H' ? 'strx.stake_staking_6hours_apy' : 'strx.stake_staking_7days_apy'
                              )}
                            </div>
                          </div>
                        </div>
                ...
                <div className="apy-box-nostake disconnect">
                  <div className="apy-title tooltip-text-wrap">
                    <TooltipText
                      ...
                      title={
                        <>
                          {intl.get(apyRange === '6H' ? 'strx.stake_staking_apy_tip1' : 'strx.stake_staking_apy_tip2')}{' '}
                          <LinkButton href={learnUrl} target="_blank" rel="noopener noreferrer">
                            {intl.get('strx.stake_staking_apy_link')}
                          </LinkButton>
                        </>
