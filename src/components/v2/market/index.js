...
const PAGE_SIZE = 300;
const { miningSymbol, jtrxAddress } = Config;

const AnnouncementTitleRegExp = {
  'en-us': /JustLend DAO Launches Supply Mining Phase/,
  'zh-cn': /JustLend DAO 开启(第[^\\x00-\\xff]+期)存款挖矿活动/,
  'zh-tc': /JustLend DAO 开启(第[^\\x00-\\xff]+期)存款挖矿活动/
};
...
class Market extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    // const regexp = AnnouncementTitleRegExp[lang.toLowerCase()] || AnnouncementTitleRegExp['en-us'];
    const regexp = AnnouncementTitleRegExp['en-us'];
    const announcement = announcementList.find(item => {
      return regexp.test(item.title);
    });
    // if (announcement && lang.toLowerCase() === 'zh-tc') {
    //   const match = announcement.title.match(regexp);
    //   if (match?.[1]) {

    //   }
    // }
    // this.setState({ announcement: announcement || announcementList?.[0] || {} });
  ...
  renderRewards() {
    ...
    return (
      <div className={'market-summary-block reward-block ' + (openMint ? '' : 'csc-box')}>
        ...
        {openMint ? (
          <div className="reward-amount">{formatNumber(marketData?.farmRewardUSD24h, 0, { needDolar: true })}</div>
        ) : (
  ...
  getInfoColumns = () => {
    const { assetList, marketList, trxPrice, noService, riojBalance, theme, openMint, userList } = this.props.lend;
    ...
    let columns = [
                  ...
                  <Tooltip
                    title={
                      text === 'SUNOLD'
                        ...
                        : intl.getHTML(`risk_tip.${text?.toLocaleLowerCase()}_icon`, { link: Config.announceLink })
                    }
                    placement="top"
                    trigger="['hover','click']"
                    arrowPointAtCenter
      ...
      },
      {
        ...
        render: (text, item) => {
          const { depositApy, mintApy, mintApyWithUSDD, totalApy } = getTotalApy(item, assetList);
      ...
      },
      {
        ...
        sorter: (b, a) => {
          if (marketList[b.jtokenAddress] && marketList[a.jtokenAddress])
            return marketLendAvailable(marketList[b.jtokenAddress]).minus(
              marketLendAvailable(marketList[a.jtokenAddress])
            );
        },
        showSorterTooltip: false,
        // width: 170,
                  ...
                  >
                    <div>
                      <div className="pipe">
                        {/* for test */}
                        {/* <div>
                    {toFixedDown(marketLendAvailable(marketList[item.jtokenAddress]), 2)} {' '}
                    {toFixedDown(marketList[item.jtokenAddress].borrowLimit, 2)} {' '}
                    {toFixedDown(marketList[item.jtokenAddress].totalSupply, 2)}{'  '}
                    {item.collateralDecimal}
                  </div> */}
                        <div
      ...
      },
      {
        ...
        sorter: (b, a) => {
          if (marketList[b.jtokenAddress] && marketList[a.jtokenAddress])
            return BigNumber(marketList[b.jtokenAddress].assetPrice)
              .times(BigNumber(10).pow(marketList[b.jtokenAddress].collateralDecimal))
              .minus(
                BigNumber(marketList[a.jtokenAddress].assetPrice).times(
                  BigNumber(10).pow(marketList[a.jtokenAddress].collateralDecimal)
                )
              );
        },
        showSorterTooltip: false,
  ...
  render() {
    ...
    let { marketDataSource, assetList, openMint } = this.props.lend;
    ...
    return isLoading ? (
      ...
      <div
        className={
          'j-market ' +
          (mobile
            ? 'market-container mobile-right-container'
            : menuFlag
            ? 'market-container'
            : 'market-container max-width')
        }
      >
