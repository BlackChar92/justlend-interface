...
class MarketSummaryV2 extends React.Component {
  ...
  setSort = value => {
    ...
    const { assetList, marketList, trxPrice } = this.props.lend;
    ...
    let marketsNew = markets.map(item => {
      ...
      return {
        ...
        detail_price: BigNumber(assetPrice).times(trxPrice).times(BigNumber(10).pow(collateralDecimal)).toNumber(),
    ...
    if (value === 'default') {
      ...
      dataArr = marketsNew.sort((a, b) => (isDesc ? b[sortKey] - a[sortKey] : a[sortKey] - b[sortKey]));
    }
  ...
  marketCardRender = dataSource => {
    ...
    const { assetList, marketList, trxPrice, theme, noService, riojBalance, openMint, userList } = this.props.lend;
    ...
    return (
      <div className="market-cards">
        {fullMarketList.map(item => {
          const { mintApy } = getTotalApy(item, assetList);
            ...
            <div
              ...
              onClick={e => {
                ...
                if (classList) {
                  ...
                  if (className.includes('j-not-used') || className.includes('not-used-icon')) return;
                }
