...
class LiquidateModal extends React.Component {
  ...
  listInit = (switchValue = this.state.switchValue) => {
    ...
    let borrowTokenList = borrowTokenListNew.slice().sort((a, b) => b.valueUsd - a.valueUsd);
  ...
  liquidateHide = () => {
    this.props.lend.setLiquidateShow(false);
    window.gtag('event', 'liquidate_liquidatepop_click_Stable', {
      'event_category': 'liquidate',
  ...
  clickMax = () => {
    ...
    window.gtag('event', 'liquidate_liquidatepop_click_inputMax', {
      'event_category': 'liquidate',
  ...
  approve = async (token = this.state.repaySelectValue) => {
    try {
      ...
      if (txID) {
        ...
        window.gtag('event', 'liquidate_liquidatepop_click_Approve', {
          'event_category': 'liquidate',
  ...
  submit = async () => {
    try {
      this.props.system.clearRejectError();
      window.gtag('event', 'liquidate_liquidatepop_click_Liquidate', {
        'event_category': 'liquidate',
      ...
      if (txID === -1) {
        window.gtag('event', 'liquidate_errorpop', {
          'event_category': 'liquidate',
          'event_label': 'liquidate_errorpop'
        });
        this.props.lend.setExceptionVisible(true);
        this.setState({ approving: false });
        return;
      }
      ...
      if (txID) {
        ...
        window.gtag('event', 'liquidate_success_pop', {
          'event_category': 'liquidate',
          'event_label': 'liquidate_success_pop'
        });
      } else {
        window.gtag('event', 'liquidate_timeout_pop', {
          'event_category': 'liquidate',
          'event_label': 'liquidate_timeout_pop'
        });
      }
      ...
      window.gtag('event', 'liquidate_failed_pop', {
        'event_category': 'liquidate',
        'event_label': 'liquidate_failed_pop'
      });
      this.setState({ approving: false });
    }
  };
  ...
  collateralTokenListSort = (collateralTokenList, switchValue = this.state.switchValue) => {
    ...
    collateralTokenList = collateralTokenList.slice().sort((a, b) => b.valueUsd - a.valueUsd);

    if (switchValue) {
      // usdd usdt tusd usdc usdj busd wstUSDT usd1
      let stableTokenList = ['jUSDD', 'jUSDT', 'jTUSD', 'jUSDC', 'jUSDJ', 'jBUSD', 'jwstUSDT', 'jUSD1'];
  ...
  render() {
    ...
    const collateralUsd = BigNumber(toBigNumberNew(repayValue).times(repayPrice).div(colleralPrice)).times(
      colleralPrice
    );
    const benefitAmount = BigNumber(toBigNumberNew(repayValue).times(repayPrice).div(colleralPrice)).times(0.08);
    const benefitUsd = BigNumber(benefitAmount).times(colleralPrice);
    const totalGetAmount = BigNumber(toBigNumberNew(repayValue).times(repayPrice).div(colleralPrice)).plus(
      benefitAmount
    );
            ...
            <Input
              ...
              onChange={e => {
                ...
                if (e.type === 'click') {
                  ...
                  window.gtag('event', 'liquidate_liquidatepop_click_inputClose', {
                    'event_category': 'liquidate',
