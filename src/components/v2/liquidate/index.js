...
import ToPCModal from './ToPCModal';
...
class Liquidate extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      marketDataSource: [{ account: 'assdasdasd', collateral: '1212', collateral: [''] }],
      checkAll: true,
  ...
  componentDidMount = async () => {
    document.title = 'Liquidation - JustLend DAO';
  ...
  disclaimerShowJudge = () => {
    ...
    if (!isStoraged) {
      if (obj.disclaimer === 'no') {
        this.props.lend.setData({ isDisclaimerStoraged: false });
      } else {
  ...
  getUserData = async () => {
    ...
    if (isConnected) {
      // this.getMarketData();
      // if (this.props.network.routeName !== 'liquidate') return;
      // this.disclaimerShowJudge();
    ...
    let columns = [
      ...
      },
      {
        title: (
          <span
            onClick={() => {
              window.gtag('event', 'click', { 'event_category': 'liquidate', 'event_label': 'liquidate_click_Debt' });
            }}
            className="ant-table-column-title"
          >
            <Tooltip
  ...
  liquidateClick = item => {
    ...
    this.props.lend.setData({ liquidateShow: true });
    window.gtag('event', 'click', { 'event_category': 'liquidate', 'event_label': 'liquidate_click_Liquidate' });
  };
  ...
  riskFilterMobileRender = () => {
    ...
    // const { theme, minRiskValue, maxRiskValue } = this.props.lend;
  ...
  riskFilterConfirm = () => {
    ...
    // if (errMsg || errMsg1 || (BigNumber(minRiskValue).isNaN() && BigNumber(maxRiskValue).isNaN())) return;
    ...
    // console.log(accounts, 'accounts', minRiskValue, maxRiskValue);

    this.props.lend.setData({
      liquidateInfo: { ...liquidateInfo, accounts },
      minRiskValue: minValue,
      maxRiskValue: maxValue
    });
    window.gtag('event', 'click', { 'event_category': 'liquidate', 'event_label': 'liquidate_click_Screen' });

    // const { theme, liquidateInfo, liquidateShow } = this.props.lend;
  };
    ...
    try {
      ...
      if (valid) {
        ...
        if (BigNumber(str).isNaN()) {
          ...
          errMsg = '';
          // this.props.lend.setData({ minRiskValue: str });
        }
    ...
    try {
      ...
      if (valid) {
        ...
        if (BigNumber(str).isNaN()) {
          ...
          // this.props.lend.setData({ maxRiskValue: str });
          errMsg1 = '';
        }
        this.setState({ errMsg1 });
      }
  ...
  totalCollateralContentRender = () => {
    const { checkAll, defaultValueCollateral } = this.state;
    const totalCollateralArr = Config.totalDebtTokenArr.map(item => {
      return 'j' + item;
    });
  ...
  onCheckAllChangeCollateral = checked => {
    ...
    if (!checkAll) {
      ...
      let arr = Config.totalDebtTokenArr.map(item => 'j' + item);
  ...
  onChangeCollateral = item => {
    ...
    let dataLength = Config.totalDebtTokenArr.length;
  ...
  totalCollateralRender = () => {
    ...
    return this.popoverRender(
      ...
      defaultValueCollateral.length !== Config.totalDebtTokenArr.length,
      intl.get('liquidate.liquidate_collateral')
    );
  };
  ...
  totalDebtRender = () => {
    ...
    return this.popoverRender(
      ...
      defaultValueDebt.length !== Config.totalDebtTokenArr.length,
      intl.get('liquidate.liquidate_debt')
    );
  };
  ...
  totalDebtContentRender = () => {
    const { checkAllDebt, defaultValueDebt } = this.state;
    return (
      <>
        <Checkbox
          className="j-check-all j-checkbox"
        ...
        >
          {Config.totalDebtTokenArr.map(item => (
            <div className="j-liquidate-item" key={item}>
              <Checkbox className="j-checkbox" value={item}>
                <div className="flex aic">
  ...
  onCheckAllChangeDebt = checked => {
    ...
    if (!checkAllDebt) {
      ...
      this.setState({ defaultValueDebt: Config.totalDebtTokenArr });
    }
  ...
  reset = () => {
    ...
    this.onCheckAllChangeCollateral(true);
    this.props.lend.setData({
      liquidateInfo: this.props.lend.liquidateOriginalInfo,
      minRiskValue: '',
      maxRiskValue: ''
    });
    ...
    window.gtag('event', 'click', { 'event_category': 'liquidate', 'event_label': 'liquidate_click_Reset' });
  };

  refreshDateFn = async () => {
    await this.props.lend.getLiquidateInfo();
    this.setState({ refreshDate: moment().format('YYYY-MM-DD HH:mm') });
    window.gtag('event', 'click', { 'event_category': 'liquidate', 'event_label': 'liquidate_click_Refresh' });
  };
      ...
      <div className="j-container pr">
        ...
        <div className="liquidate-warning">
          <span>
            ...
            <a href="https://docs.justlend.org/resources/risk-warning" className="jl-links" target="disclaimer">
          ...
          return (
            <div className="liquidate-m-item" key={item.borrower}>
              ...
              {/* <div
                ...
                onClick={() => {
                  this.setState({ dataInfo: item }), this.props.lend.setData({ liquidateShow: true });
                }}
              >
                {intl.get('liquidate.liquidate_liquidate')}
              </div> */}
  ...
  //   const { mobile } = this.state;
  //   return (
  //     <div className={'j-container pr' + (mobile ? ' j-liquidate-container-m' : '')}>
  //       <div className="j-navi">
  //         <div className="j-navi-content">
  //           <span className="current">{intl.get('application.liquidation')}</span>
  //           <div>{intl.get('application.des4')}</div>
  ...
  //       <div className="liquidate-disclaimer">
  //         <div className="ld-icon"></div>
  //         <div className="ld-desc">{intl.get('disclaimer.agree1')}</div>
  //         <div className="j-btn j-rent-renew" onClick={() => this.props.lend.setData({ disclaimerShow: true })}>
  //           {intl.get('disclaimer.agree3')}
  //         </div>
  //       </div>
  //     </div>
  //   );
  ...
  render() {
    ...
    let isLoading = true;
    if (liquidateOriginalInfo?.accounts?.length && (marketDataSource?.length || marketDataSource.length === 0)) {
      isLoading = false;
    }
