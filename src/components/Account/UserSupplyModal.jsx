...
const UserSupplyModal = observer(({}) => {
  const [mobile] = useState(isMobile(window.navigator).any);

  const { user, ui, lend } = Stores;

  const getOption = () => {
    const { usddJtoken, usddoldJtoken } = Config;
    let { theme } = lend;
    let { totalSupplyUsd, userDepositAndJustMortgateDataSource: dataSource } = user;
    const colors = [
      '#735ff6',
      '#4c54ff',
      '#9195fb',
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={() => ui.setUserSupplyModalShow(false)}
          ...
          {supplyList?.length > 0 && (
            <div className="user-invest-modal-overview flexBA">
              <div>
                ...
                <p className="overview-val">{getTotalDeposit()}</p>
              </div>
              <div className="right">
