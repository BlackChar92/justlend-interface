...
class Borrow extends React.Component {
  ...
  componentDidMount = () => {
    this.startInterval();
    const { borrowModalInfo } = this.props.lend;
    const { type } = borrowModalInfo;
    const { totalBorrowUsdForUSDD } = this.props.user;
    ...
    if (type === '2') {
      ...
      if (!hasApproved) {
        window.gtag('event', 'PC_borrow_approve1tips', {
  ...
  pollCash = async () => {
    if (this.isFetchingCash) return;
    this.isFetchingCash = true;
    try {
      await this.getCash();
    } finally {
      this.isFetchingCash = false;
  ...
  onChangeBorrow = async (inputValue, maxValue = this.state.maxValue) => {
    try {
      ...
      const { borrowModalInfo } = this.props.lend;
      const { userList, totalBorrowValueInTrx, totalCollateralValueInTrx, totalBorrowUsdForUSDD, borrowLimit } =
        this.props.user;
      ...
      if (valid) {
        ...
        } else if (BigNumber(maxValue).eq(1)) {
          status = true;
  ...
  getTotalBorrowUsdAfter = (value = 0, isRepay) => {
    try {
      const { borrowModalInfo } = this.props.lend;
      const { userList, totalBorrowUsdForUSDD } = this.props.user;
      const { marketList, trxPrice } = this.props.market;
      const { jtokenAddress } = borrowModalInfo;
  ...
  onChangeRepay = (inputValue, fromInput = false) => {
    try {
      ...
      const { borrowModalInfo } = this.props.lend;
      const { userList, totalBorrowUsdForUSDD } = this.props.user;
      const { marketList, balanceInfo } = this.props.market;
      const { jtokenAddress } = borrowModalInfo;
      const popData = userList[jtokenAddress] || marketList[jtokenAddress];
      ...
      if (valid) {
        ...
        const { approved } = this.getApprovedLimit(undefined, undefined, str);
        if (!approved) {
          window.gtag('event', 'PC_borrow_approve2tips', {
            'event_category': 'PC_V1.7.6',
            'event_label': 'borrow_approve2tips'
          });
        }

        if (!BigNumber(inputValue).eq(0)) {
  ...
  renderBorrow = () => {
    const { borrowModalInfo, theme } = this.props.lend;
    const {
      userList,
      risk,
    ...
    const { marketList, noService, riojBalance } = this.props.market;
  ...
  collateralOn = async transInfo => {
    ...
    this.props.lend.setMortgageModalInfo({ visible: true, type: 1, jtokenAddress: transInfo?.jtokenAddress });
  };
  ...
  getApprovedLimit = (token, value1, value2) => {
    const { repayValue, wstUSDTSelectedToken, wstUSDTInputValue } = this.state;
    const { borrowModalInfo, wstUSDTbalanceInfo } = this.props.lend;
    const { userList } = this.props.user;
    const { marketList, balanceInfo } = this.props.market;
    const { jtokenAddress } = borrowModalInfo;
    const popData = userList[jtokenAddress] || marketList[jtokenAddress];
    const wsToken = token || wstUSDTSelectedToken;
    const wValue = value1 || wstUSDTInputValue;
    const rValue = value2 || repayValue;

    let approved = false;
    let hasApproved = false;

    if (popData.collateralSymbol !== 'wstUSDT' || wsToken === 'wstusdt') {
      hasApproved = balanceInfo[jtokenAddress] && BigNumber(balanceInfo[jtokenAddress].allowance).gt(0);
      const allowance = balanceInfo[jtokenAddress]?.allowance;

      approved =
        allowance &&
        BigNumber(allowance).gt(0) &&
        BigNumber(allowance)?.gte(BigNumber(BigNumber(rValue).isNaN() ? 0 : rValue).times(popData.precision));
    } else {
  ...
  renderRepay = () => {
    ...
    const { theme, wstUSDTbalanceInfo } = this.props.lend;
    const { isConnected } = this.props.network;
    ...
    const { marketList, balanceInfo, noService, riojBalance } = this.props.market;
    const { jtokenAddress } = this.props.lend.borrowModalInfo;
    const popData = userList[jtokenAddress] || marketList[jtokenAddress];
    const {
      precision,
    ...
    let per2 =
      isMax && !BigNumber(repayValue).eq(0)
        ? 0
    ...
    const status2 =
      repayValue !== '' && !BigNumber(repayValue).eq(0) && (!BigNumber(per1).eq(0) || !BigNumber(per2).eq(0));
              ...
              <>
                <Button
                  ...
                  onClick={async () => {
                    ...
                    // TRC-20 USDT approve race guard: thread the current allowance
                    // onto popData so lendApprove/safeApprove can reset first.
                    popData.currentAllowance =
                      this.props.market.balanceInfo?.[popData.jtokenAddress]?.allowance;
  ...
  changeTab = value => {
    ...
    if (popData && jtokenAddress && balanceInfo[jtokenAddress]) {
      this.props.lend.setBorrowModalInfo({
        type: value,
        visible: true,
        jtokenAddress
      });
  ...
  onChangeWstUSDTRepay = (wstUSDTSelectedToken, repayValue, wstUSDTInputValue, errInfo2, isClear) => {
    try {
      ...
      const { approved } = this.getApprovedLimit(wstUSDTSelectedToken, wstUSDTInputValue, repayValue);
      if (!approved) {
        window.gtag('event', 'PC_borrow_approve2tips', {
          'event_category': 'PC_V1.7.6',
          'event_label': 'borrow_approve2tips'
