...
class Mortgage extends React.Component {
  ...
  getData = (mortgageModalInfo, userList, borrowLimit, totalBorrowUsd) => {
    ...
    try {
      const { risk, totalBorrowValueInTrx, totalCollateralValueInTrx, marketList } = this.props.lend;
  ...
  renderContent = (mortgageModalInfo, userList, borrowLimit, totalBorrowUsd, collateralSymbol) => {
    ...
    //   }
    // });
    // let isOneLeft = type === 2 && isOpenCount === 1;
    // const disableStatus = BigNumber(per2).gte(100) || isOneLeft;
      ...
      <>
        ...
        {this.state.approving ? (
          ...
          >
            ...
            {/* {type === 1
              ? intl.get('v2.enable_collateral', { symbol: collateralSymbol })
              : disableStatus
              ? intl.get('v2.in_use')
              : intl.get('v2.disable_collateral_symbol', { symbol: collateralSymbol })} */}
          </Button>
  ...
  render() {
    const { mortgageModalInfo, userList, borrowLimit, totalBorrowUsdForUSDD } = this.props.lend;
