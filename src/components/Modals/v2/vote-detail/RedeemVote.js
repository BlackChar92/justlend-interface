...
class RedeemVote extends React.Component {
  ...
  withdrawVotes = async () => {
    ...
    const { lockNum, redeemFromVotePopProposalId } = this.props.lend;

    // if (!lockNum || !redeemFromVotePopProposalId || lockNum.lte(0)) {
  ...
  render() {
    const { redeemFromVotePop, lockNum } = this.props.lend;
