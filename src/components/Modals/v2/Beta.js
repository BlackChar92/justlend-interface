...
class BetaModal extends React.Component {
  ...
  close = () => {
    ...
    this.props.lend.setData({ betaModalVisible: false });
  ...
  render() {
    ...
    const { theme, betaModalVisible, betaInfo, applicationMap } = this.props.lend;
    const { defaultAccount } = this.props.network;
    // const hasSeenBeta = window.localStorage.getItem('hasSeenBeta' + defaultAccount);
    // const hasStoreSeenBeta = !hasSeenBeta;
    // const isDiff = hasSeenBeta !== JSON.stringify(betaInfo) && JSON.stringify(betaInfo) !== '[]';
    //
    let isExpired = false;
    betaInfo?.map(item => {
      if (item.status === 3) {
